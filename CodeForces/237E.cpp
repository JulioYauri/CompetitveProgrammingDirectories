...
signed main() {
    ...
    string t; cin >> t; 
    int n; cin >> n; 
    MCMF solver(n + 26 + 1 + 1); 
    int src = 0, sink = n + 27; 
    rep(i,1,n+1) { 
        string s; cin >> s; 
        int lim ; cin >> lim; 
        solver.addEdge(i + 26, sink, lim, 0); 
        for(char c = 'a'; c <= 'z'; c++) { 
            int cnt = count(all(s), c); 
            if(cnt) solver.addEdge(int(c - 'a') + 1, i + 26, cnt, i); 
        }
    }

    for(char c = 'a'; c <= 'z'; c++) { 
        int cnt = count(all(t), c); 
        if(cnt) solver.addEdge(src, int(c - 'a') + 1, cnt, 0); 
    }

    auto [flow, cost] = solver.maxflow(src, sink);
    if(flow < sz(t)) cout << "-1\n"; 
    else cout << cost << "\n"; 
